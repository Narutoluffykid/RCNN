import numpy as np
from RCNN import selectivesearch
import os.path
from sklearn import svm
from sklearn.externals import joblib
from RCNN import preprocessing_RCNN as prep
import os
from RCNN import tools
import cv2
...
def train_svms(train_file_folder, model):
    ...
    svms = []
    #为每一种类型训练一个svm，原文一共训练了21个svm,包括背景。此处仅训练了2个
    for train_file in files:
        if train_file.split('.')[-1] == 'txt':
            X, Y = generate_single_svm_train(train_file_folder+'/'+train_file)  #获得svm的训练集和标签
            train_features = []
            #用Alexnet提取图片的特征向量，经过Alexnet后输出维度为（1746，4096）
...
if __name__ == '__main__':
    ...
    img_path = './17flowers/jpg/image_0534.jpg'  # or './17flowers/jpg/16/****.jpg'
    imgs, verts = image_proposal(img_path)      #得到region proposal
    #tools.show_rect(img_path, verts)            #显示region proposal

    net = create_alexnet()
    model = tflearn.DNN(net)
    model.load(config.FINE_TUNE_MODEL_PATH)     #载入fine_tune之后的权重参数
    svms = []
