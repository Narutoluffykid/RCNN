...
# Read in data and save data for Alexnet and svm
def load_train_proposals(datafile, num_clss, save_path, threshold=0.5, is_svm=False, save=False):
    ...
    for num, line in enumerate(train_list):
        ...
        for r in regions:
            ...
            iou_val = IOU(ref_rect_int, proposal_vertice)   #计算真实标记框与建议框（基于图像分割给出的region proposal）的IOU
            ...
            if is_svm:
                ...
                label = np.zeros(num_clss + 1)      #SVM以向量的方式存储标签数据
                if iou_val < threshold:
                    label[0] = 1
                else:
                    label[index] = 1
